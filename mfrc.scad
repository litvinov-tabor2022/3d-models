...
board_size = [59.9, 39.4, 1.5];
board_col_height = 5.6;

header_size = [7, 10.5, 4.9];
header_pos = [19.1, board_size.z];

header2_size = [15.6, 14.9, 5.95];

shaft_length_diff = 4.4;
rails_offset = 33.2;
...
inner_box = [board_size.x + 2 * inset, board_size.y + 2 * inset, board_col_height + 2 * inset];
...
cover_size_outter = [cover_size_inner.x + 2 * fatness, cover_size_inner.y + 2 * fatness, cover_size_inner.z + fatness];

shaft_size_inner = [
        cover_size_outter.x + shaft_length_diff,
        cover_size_outter.y + 2 * shaft_inset,
        cover_size_outter.z + shaft_inset
    ];
...
cs_size = [header2_size.x + 4, shaft_size_outter.y, shaft_size_outter.z];

echo("MFRC shatf size outter:", shaft_size_outter);

header_hole_size = [header_size.x + .4, header_size.y + .4, header_size.z + .4];
...
module MFRC_inner(x, y, z) {
    union() {
        difference() {
            ...
            translate([- .01, fatness + board_size.y - header_pos.x - header_hole_size.y + .4, fatness + header_pos.y - .2]) {
        ...
        if (DEBUG) MFRC_board(fatness + inset, fatness + inset, fatness);
    }
}

module MFRC_cover(x, y, z) {
    union() {
        difference() {
            union() {
                difference() {
            ...
            translate([- .01, 2 * fatness + header_pos.x + inset / 2, fatness + .3]) {
                cube(header_hole_size);
            }

            if (DEBUG) translate([- .01, fatness, fatness]) cube([fatness + .02, box.y - 2 * fatness, 100]);
            if (DEBUG) translate([fatness, fatness, - .1]) cube([cover_size_inner.x, cover_size_inner.y, 100]);
        }

        supp_size = [3, 3, cover_size_outter.z - 2 * fatness - board_size.z - .1];
        translate([15, (cover_size_inner.y - supp_size.y) / 2, fatness]) cube(supp_size);
        translate([40, (cover_size_inner.y - supp_size.y) / 2, fatness]) cube(supp_size);
        ...
        difference() {
            union() {
                translate([cover_size_outter.x, .01, cover_size_outter.z / 2]) rotate([90])
                    cylinder(h = 2.6, d = holds_dia, $fn = round_prec);
                translate([cover_size_outter.x, cover_size_outter.y + 2.6, cover_size_outter.z / 2]) rotate([90])
                    cylinder(h = 2.6, d = holds_dia, $fn = round_prec);
            }
...
module MFRC_shaft() {
    translate([cs_size.x, 0]) union() {
        ...
        // conector shaft
        translate([- cs_size.x, 0, 0]) union() {

            header_hole_offset = 14.1;
            ...
            difference() {
                ...
                translate([cs_size.x - fatness - .01, header_hole_offset, 2]) {
            ...
            // header
            translate([cs_size.x - header2_size.x - fatness, header_hole_offset + (header_hole_size.y - header2_size.y) / 2, fatness]
            ) {
                if (DEBUG) color("black") translate([0, 0, 2.4]) cube(header2_size);

                difference() {
                    w = 4;
                    h = 6.3;
                    translate([- 3, - 3, - .01]) difference() {
                        cube([3 + header2_size.x, 2 * 3 + header2_size.y, h]);
                        translate([- .01, 3 + w, - .01]) cube([3 + .02, header2_size.y - 2 * w, h + .02]);
                    }
                    translate([0, 0, 2.1]) cube([header2_size.x + .02, header2_size.y + .02, header2_size.z + .02]);
                }
            }
        }

        if (DEBUG) MFRC_board(6.3, 4.5, 1.8);
    }
}
...
//    MFRC_inner(0, 0, 0);
//
//    translate([- fatness - inset / 2, - 17.7, box.z + fatness + inset]) rotate([180])
//        MFRC_cover(0, - 60, 0);
//
//    translate([- 5.3, 116.5, - 1])
//        MFRC_shaft(0, - 120, 0);
